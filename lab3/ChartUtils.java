...
public class ChartUtils {
    ...
    public static JPanel createSpectrogramChartPanel(double[][] spectrogram, String title, int sampleRate, int hopSize,
        ...
        int bins = frames > 0 ? spectrogram[0].length : 0;
        if (frames == 0 || bins == 0) {
            panel.add(new JLabel("Нет данных для спектрограммы", SwingConstants.CENTER), BorderLayout.CENTER);
            return panel;
        }
        ...
        for (int t = 0; t < frames; t++) {
            for (int f = 0; f < bins; f++) {
                double db = 20.0 * Math.log10(spectrogram[t][f] + 1e-12);
                if (db < minDb) minDb = db;
                if (db > maxDb) maxDb = db;
                x[index] = t * (hopSize / (double) sampleRate);
                y[index] = f * (sampleRate / 2.0) / Math.max(1, bins - 1);
                z[index] = db;
                index++;
            }
        }
        ...
        dataset.addSeries("Спектрограмма", new double[][]{x, y, z});

        NumberAxis xAxis = new NumberAxis("Время, с");
        NumberAxis yAxis = new NumberAxis("Частота, Гц");
    ...
    private static PaintScale createSpectrogramPaintScale(double min, double max) {
        if (max <= min) {
            max = min + 1.0;
        }
        LookupPaintScale scale = new LookupPaintScale(min, max, new Color(10, 10, 40));
        for (int i = 0; i <= 255; i++) {
            double ratio = i / 255.0;
            int r = (int) (255 * Math.pow(ratio, 0.9));
            int g = (int) (180 * Math.pow(ratio, 1.1));
            int b = (int) (255 * (1.0 - ratio * 0.8));
            scale.add(min + ratio * (max - min), new Color(clamp(r), clamp(g), clamp(b)));
        }
        return scale;
    }
